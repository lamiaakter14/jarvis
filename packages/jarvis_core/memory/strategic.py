    ...
    def get_snapshot_history(
        ...
        try:
            ...
            raise RepositoryError(
                f"Failed to retrieve snapshot history for goal {goal_id}: {e}"
            )
...
def trajectory_delta(
    ...
    prev_keys = {k for k in previous_snapshot.keys() if not k.startswith('_')}
    curr_keys = {k for k in current_snapshot.keys() if not k.startswith('_')}
