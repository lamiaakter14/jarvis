...
class MetricsCollector:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        total_agent_executions = sum(
            m.get("total_executions", 0) for m in agent_metrics.values()
        )
        total_agent_success = sum(
            m.get("successful_executions", 0) for m in agent_metrics.values()
        )
        
        agent_success_rate = (
            total_agent_success / total_agent_executions
