...
from jarvis_core.shared.exceptions import EntityNotFoundError, RepositoryError
...
class FileMemoryRepository(IMemoryRepository):
    ...
    async def search(
        ...
        try:
            ...
            if key_pattern:
                filtered_memories = [
                    m for m in filtered_memories if key_pattern.lower() in m.key.lower()
                ]
            ...
            if keywords:
                ...
                filtered_memories = [m for m in filtered_memories if matches_keywords(m)]

            # Filter by tags
            if tags:
                filtered_memories = [
                    m for m in filtered_memories if any(tag in m.get_tags() for tag in tags)
                ]
    ...
    def _update_strategic_index(self, memory: Memory) -> None:
        ...
        if entry_type:
            self.strategic_index.add_entry(
                key=memory.key, entry_type=entry_type, priority=priority, status=status, tags=tags
            )
