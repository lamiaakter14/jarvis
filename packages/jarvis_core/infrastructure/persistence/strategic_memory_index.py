...
class StrategicMemoryIndex:
    ...
    def _load_index(self) -> None:
        ...
        try:
            ...
            # Restore indexes
            self.by_priority = {
                k: set(v) for k, v in data.get('by_priority', {}).items()
            }
            self.by_status = {
                k: set(v) for k, v in data.get('by_status', {}).items()
            }
            self.by_tag = {
                k: set(v) for k, v in data.get('by_tag', {}).items()
            }
            self.by_type = {
                k: set(v) for k, v in data.get('by_type', {}).items()
            }
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'by_type': {
                entry_type: len(keys) 
                for entry_type, keys in self.by_type.items()
            },
            'by_priority': {
                priority: len(keys)
                for priority, keys in self.by_priority.items()
            },
            'by_status': {
                status: len(keys)
                for status, keys in self.by_status.items()
            },
