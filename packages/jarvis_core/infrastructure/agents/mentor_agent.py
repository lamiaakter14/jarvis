...
class MentorAgent(Agent):
    ...
    def _generate_recommendations(self, gaps: list[dict]) -> list[str]:
        ...
        if high_severity:
            recommendations.append(f"Address {len(high_severity)} high-severity gaps immediately")
            for gap in high_severity:
                recommendations.append(f"- {gap.get('description')}")

        if medium_severity:
            recommendations.append(f"Plan to address {len(medium_severity)} medium-severity gaps")
        ...
        if skill_gaps:
            recommendations.append(f"Practice and training needed for {len(skill_gaps)} skill gaps")
