...
class ManageStrategicMemory:
    ...
    async def list_active_goals(self) -> List[Memory]:
        ...
        try:
            ...
            active_goals = [
                m for m in all_strategic
                if m.content.get("status") == "active"
                and "goal" in m.content
            ]
            
            # Sort by priority (critical > high > medium > low)
            priority_order = {"critical": 0, "high": 1, "medium": 2, "low": 3}
            active_goals.sort(
                key=lambda m: priority_order.get(m.content.get("priority", "low"), 3)
            )
    ...
    async def list_adrs(self, status: Optional[str] = None) -> List[Memory]:
        ...
        try:
            ...
            if status:
                adrs = [
                    m for m in adrs
                    if m.content.get("status") == status
                ]
            
            # Sort by date (newest first)
            adrs.sort(
                key=lambda m: m.content.get("date", datetime.min),
                reverse=True
            )
