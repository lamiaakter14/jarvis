...
class ExecuteTasks:
    ...
    def __init__(
        self, task_repository: ITaskRepository, notification_service: INotificationService
    ):
        ...
        for task_id in task_ids:
            try:
                ...
                await self.notification_service.send_notification(
                    f"Failed to execute task {task_id}: {str(e)}", "error"
                )
                continue
    ...
    async def get_tasks_by_priority(
        ...
        filtered_tasks = [task for task in all_tasks if task.priority.value == priority.value]
