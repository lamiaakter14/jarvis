...
class AnalyzePerformance:
    ...
    async def execute(self, start_date: date, end_date: date) -> AnalyticsDTO:
        ...
        try:
            ...
            tasks_in_period = [
                t for t in all_tasks if start_date <= t.created_at.date() <= end_date
            ]
            ...
            average_roi = (
                sum(t.roi.value for t in tasks_in_period) / total_tasks if total_tasks > 0 else 0.0
            )

            # Calculate time metrics
            total_hours = sum(
                t.cognitive_load.estimated_hours for t in tasks_in_period if t.is_completed()
            )

            completed_with_duration = [
                t for t in tasks_in_period if t.is_completed() and t.calculate_duration()
            ]
    ...
    async def _get_top_gaps(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        sorted_gaps = sorted(
            gaps_data, key=lambda g: severity_order.get(g.get("severity", "low"), 0), reverse=True
        )
