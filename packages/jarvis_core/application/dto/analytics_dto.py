...
class AnalyticsDTO(BaseModel):
    ...
    period: str = Field(
        ..., description="Time period for analytics (e.g., 'daily', 'weekly', 'monthly')"
    )
    ...
    productivity_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Overall productivity score"
    )
    ...
    average_task_duration: float = Field(
        default=0.0, ge=0.0, description="Average task duration in hours"
    )
