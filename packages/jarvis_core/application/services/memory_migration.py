...
class MemoryMigration:
    ...
    def validate_and_fix_memory(memory: Memory) -> tuple[Memory, list[str]]:
        ...
        try:
            ...
            fixed_content = MemoryMigration._apply_fixes(
                memory_type_str, memory.content, fixes_applied
            )
            ...
            try:
                ...
                raise MigrationError(f"Failed to fix memory {memory.key}: {str(e)}")
    ...
    def _apply_fixes(
        ...
        if memory_type == "strategic":
            fixed_content = MemoryMigration._fix_strategic_memory(fixed_content, fixes_log)
        elif memory_type == "adr":
            fixed_content = MemoryMigration._fix_adr_memory(fixed_content, fixes_log)
        elif memory_type == "knowledge":
            fixed_content = MemoryMigration._fix_knowledge_memory(fixed_content, fixes_log)
        elif memory_type == "execution_log":
            fixed_content = MemoryMigration._fix_execution_log(fixed_content, fixes_log)
        elif memory_type == "working":
            fixed_content = MemoryMigration._fix_working_memory(fixed_content, fixes_log)
    ...
    async def migrate_memory(
        ...
        for version in range(current_version + 1, target_version + 1):
            migration_func = MemoryMigration.MIGRATIONS.get(memory_type_str, {}).get(version)
    ...
    async def validate_repository_memories(
        ...
        """
        report = {"total": 0, "valid": 0, "fixed": 0, "failed": 0, "errors": []}
        ...
        for memory in memories:
            try:
                ...
                report["failed"] += 1
                report["errors"].append({"key": memory.key, "error": str(e)})
