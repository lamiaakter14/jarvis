...
class MetricsReport(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="When the metrics were calculated"
    )
    strategic_alignment_score: float = Field(
        ..., ge=0.0, le=1.0, description="Ratio of completed tasks related to primary mission"
    )
    cognitive_throughput: float = Field(
        ..., ge=0.0, description="Tasks completed per active focus hour"
    )
    learning_velocity: float = Field(..., description="Skill improvement per day")
    momentum_index: float = Field(
        ..., ge=0.0, le=1.0, description="Weighted combination of all metrics"
    )
    ...
    completed_tasks_related_to_mission: int = Field(
        default=0, ge=0, description="Tasks completed that align with primary mission"
    )
    ...
    skill_improvement_delta: float = Field(default=0.0, description="Change in skill proficiency")
...
class MetricsEngine:
    ...
    def calculate_strategic_alignment_score(
        self, completed_tasks_related_to_primary_mission: int, total_tasks: int
    ) -> float:
        ...
        if completed_tasks_related_to_primary_mission > total_tasks:
            raise ValueError("completed_tasks_related_to_primary_mission cannot exceed total_tasks")
    ...
    def calculate_cognitive_throughput(
        self, completed_tasks: int, active_focus_hours: float
    ) -> float:
    ...
    def calculate_learning_velocity(
        self, skill_improvement_delta: float, days_elapsed: float
    ) -> float:
    ...
    def calculate_metrics(
        ...
        cognitive_throughput = self.calculate_cognitive_throughput(
            completed_tasks=completed_tasks, active_focus_hours=active_focus_hours
        )

        learning_velocity = self.calculate_learning_velocity(
            skill_improvement_delta=skill_improvement_delta, days_elapsed=days_elapsed
        )
