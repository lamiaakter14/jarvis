...
class InnovationEngine:
    ...
    def analyze_patterns(self, tasks: list[Task], context: Context) -> list[Innovation]:
        ...
        inefficient_tasks = [
            t for t in tasks if t.cognitive_load.estimated_hours > 2.0 and t.roi.is_low_value()
        ]
        ...
        if context.gaps:
            high_severity_gaps = [
                g for g in context.gaps if g.get("severity") in ["high", "critical"]
            ]
    ...
    def filter_actionable_innovations(
        ...
        actionable = [
            ...
            if self.score_innovation(inn) >= min_score and (inn.is_proposed() or inn.is_approved())
        ]
