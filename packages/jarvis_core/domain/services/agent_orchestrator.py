...
class AgentOrchestrator:
    ...
    def validate_agent_compatibility(self, agents: List[Agent], context: Context) -> Dict[str, Any]:
        ...
        total_estimated_hours = sum(agent.get_average_execution_time() / 3600.0 for agent in agents)
        ...
        if len(set(agent_types)) == 1 and len(agents) > 1:
            validation["warnings"].append("All agents are the same type - consider diversifying")
        ...
        if unhealthy_agents:
            validation["warnings"].append(f"{len(unhealthy_agents)} agents have low success rates")
            validation["recommendations"].append("Review and fix failing agents before execution")
