...
class TestFileMemoryRepository:
    ...
    async def test_save_and_retrieve_memory(self, repository):
        ...
        memory = Memory(key="test_memory", type=MemoryType.WORKING, content={"data": "test_value"})
    ...
    async def test_delete_memory(self, repository):
        ...
        memory = Memory(key="delete_test", type=MemoryType.WORKING, content={"data": "delete_me"})
    ...
    async def test_search_by_keywords(self, repository):
        ...
        # Search for "Python"
        results = await repository.search(memory_type=MemoryType.KNOWLEDGE, keywords=["Python"])
    ...
    async def test_version_tracking(self, repository):
        ...
        memory = Memory(
            key="versioned_memory", type=MemoryType.KNOWLEDGE, content={"version": "1.0"}
        )
    ...
    async def test_retrieve_specific_version(self, repository):
        ...
        memory = Memory(key="version_test", type=MemoryType.KNOWLEDGE, content={"data": "v1"})
    ...
    async def test_search_pagination(self, repository):
        ...
        for i in range(15):
            memory = Memory(key=f"paginate_{i}", type=MemoryType.KNOWLEDGE, content={"index": i})
            await repository.save(memory)

        # Get first page
        page1 = await repository.search(memory_type=MemoryType.KNOWLEDGE, limit=10, offset=0)

        # Get second page
        page2 = await repository.search(memory_type=MemoryType.KNOWLEDGE, limit=10, offset=10)
