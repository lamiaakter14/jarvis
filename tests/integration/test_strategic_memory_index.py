...
class TestStrategicMemoryIndexedStorage:
    ...
    async def test_index_updated_on_goal_creation(self, use_case, repository):
        ...
        goal = await use_case.create_strategic_goal(
            goal="Test indexed goal", description="Testing index updates", priority="high"
        )
    ...
    async def test_index_search_performance(self, use_case, repository):
        ...
        results = await repository.search(memory_type=MemoryType.STRATEGIC, tags=["high"])
    ...
    async def test_index_updated_on_goal_update(self, use_case, repository):
        ...
        # Create a goal
        goal = await use_case.create_strategic_goal(goal="Test update", priority="low")
    ...
    async def test_index_removed_on_deletion(self, use_case, repository):
        ...
        # Create a goal
        goal = await use_case.create_strategic_goal(goal="Test deletion", priority="high")
    ...
    async def test_index_persistence(self, temp_memory_dir, use_case):
        ...
        goal = await use_case.create_strategic_goal(goal="Persistent goal", priority="high")
        adr = await use_case.create_adr(
    ...
    async def test_index_statistics(self, use_case, repository):
        ...
        await use_case.create_strategic_goal(goal="Goal 1", priority="high")
        await use_case.create_strategic_goal(goal="Goal 2", priority="medium")
        await use_case.create_adr(
            title="ADR 1", context="Test", decision="Test", consequences="Test", status="proposed"
        )
    ...
    async def test_search_with_multiple_tags(self, use_case, repository):
        ...
        await use_case.create_strategic_goal(goal="Backend goal", priority="high")
        await use_case.create_strategic_goal(goal="Frontend goal", priority="medium")

        # Search for high priority using tags
        results = await repository.search(
            memory_type=MemoryType.STRATEGIC, tags=["high", "strategic"]
        )
    ...
    async def test_index_with_complex_queries(self, use_case, repository):
        ...
        await use_case.create_strategic_goal(
            goal="Performance improvement", priority="critical", milestones=[{"name": "Phase 1"}]
        )
        await use_case.create_strategic_goal(goal="Security enhancement", priority="high")
        await use_case.create_adr(
        ...
        results = await repository.search(memory_type=MemoryType.STRATEGIC, tags=["strategic"])

        # All goals should have strategic tag
        assert len(results) >= 2

        # Search for ADRs specifically
        adr_results = await repository.search(memory_type=MemoryType.STRATEGIC, tags=["adr"])
