...
class TestManageStrategicMemory:
    ...
    async def test_update_goal_progress(self, use_case):
        ...
        # Create a goal
        goal = await use_case.create_strategic_goal(goal="Test Goal", priority="medium")

        # Update progress
        updated_goal = await use_case.update_goal_progress(
            goal_key=goal.key, progress=50.0, status="active"
        )
    ...
    async def test_list_active_goals(self, use_case):
        ...
        await use_case.create_strategic_goal(goal="Goal 1", priority="critical")
        await use_case.create_strategic_goal(goal="Goal 2", priority="high")

        # Create a completed goal
        completed_goal = await use_case.create_strategic_goal(goal="Goal 3", priority="medium")
        await use_case.update_goal_progress(
            goal_key=completed_goal.key, progress=100.0, status="completed"
        )
    ...
    async def test_search_strategic_memory(self, use_case):
        ...
        await use_case.create_strategic_goal(
            goal="Enhance user experience", description="Redesign UI", priority="medium"
        )

        # Search by keyword
        results = await use_case.search_strategic_memory(keywords=["performance"])
    ...
    async def test_search_by_tags(self, use_case):
        ...
        # Create goals
        await use_case.create_strategic_goal(goal="High priority goal", priority="high")
        await use_case.create_strategic_goal(goal="Medium priority goal", priority="medium")

        # Search for high priority goals
        results = await use_case.search_strategic_memory(tags=["high"])
    ...
    async def test_goal_with_dependencies(self, use_case):
        ...
        prereq_goal = await use_case.create_strategic_goal(
            goal="Setup infrastructure", priority="high"
        )

        # Create a dependent goal
        dependent_goal = await use_case.create_strategic_goal(
            goal="Deploy application", priority="high", dependencies=[prereq_goal.key]
        )
