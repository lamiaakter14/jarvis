...
class TestExecuteTasks:
    ...
    async def test_get_tasks_by_status(
        ...
        tasks = [
            Task(
                ...
                roi=ROI(0.7),
                status=TaskStatus.PENDING if i < 2 else (TaskStatus.IN_PROGRESS if i < 4 else TaskStatus.COMPLETED),
    ...
    async def test_get_tasks_by_priority_and_status(
        ...
        results = await use_case.get_tasks_by_priority(
            TaskPriority.HIGH, 
            status=TaskStatus.PENDING
        )
        ...
        assert all(
            task.priority == TaskPriority.HIGH.value and 
