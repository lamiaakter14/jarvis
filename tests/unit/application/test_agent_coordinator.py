...
from jarvis_core.shared.constants import AgentType, TaskPriority, TaskStatus
...
class TestAgentCoordinator:
    ...
    def context(self):
        """Create test context."""
        return Context(available_hours=8.0, current_focus=["Testing"])
    ...
    @pytest.mark.asyncio
    async def test_coordinate_agents_parallel(self, coordinator, context):
        ...
        agent_types = [AgentType.STRATEGIST, AgentType.EXECUTOR, AgentType.MENTOR]

        result = await coordinator.coordinate_agents_parallel(
            context, agent_types, max_concurrent=2
        )
    ...
    @pytest.mark.asyncio
    async def test_coordinate_agents_parallel_with_failure(self, coordinator, context):
        ...
        result = await coordinator.coordinate_agents_parallel(context, agent_types)
    ...
    @pytest.mark.asyncio
    async def test_synchronize_strategic_agents(self, coordinator, context):
    ...
    @pytest.mark.asyncio
    async def test_synchronize_strategic_agents_with_failure(self, coordinator, context):
