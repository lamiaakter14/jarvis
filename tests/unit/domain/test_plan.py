...
class TestPlan:
    ...
    def test_create_plan_with_required_fields(self):
        ...
        plan = Plan(plan_id=str(uuid4()), date=date.today(), tasks=[], total_hours=8.0)
    ...
    def test_remove_task_from_plan(self, sample_tasks):
        ...
        plan = Plan(
            plan_id=str(uuid4()), date=date.today(), tasks=sample_tasks.copy(), total_hours=8.0
        )
    ...
    def test_get_total_estimated_hours(self, sample_tasks):
        ...
        plan = Plan(plan_id=str(uuid4()), date=date.today(), tasks=sample_tasks, total_hours=8.0)
