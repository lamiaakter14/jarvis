...
class RateLimiter:
    ...
    def _clean_old_entries(self, bucket: Dict[str, list], max_age_seconds: int) -> None:
        ...
        for client_id in list(bucket.keys()):
            ...
            bucket[client_id] = [(ts, count) for ts, count in bucket[client_id] if ts > cutoff_time]
    ...
    def _check_limit(
        ...
        recent_requests = sum(count for ts, count in bucket[client_id] if ts > cutoff_time)
    ...
    async def __call__(self, request: Request) -> Optional[JSONResponse]:
        ...
        minute_allowed, minute_remaining = self._check_limit(
            client_id, self._minute_buckets, self.requests_per_minute, 60
        )
        ...
        hour_allowed, hour_remaining = self._check_limit(
            client_id, self._hour_buckets, self.requests_per_hour, 3600
        )
