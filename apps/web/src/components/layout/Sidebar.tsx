...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={cn('fixed top-0 left-0 z-50 h-full bg-jarvis-surface border-r border-jarvis-border transition-all duration-300 flex flex-col', 'lg:static lg:z-0', isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0', collapsed ? 'w-16' : 'w-52')}>
              ...
              {section.items.map(item => {
                ...
                return (
                  <Link key={item.name} to={item.href} onClick={onClose} title={collapsed ? item.name : undefined}
                    className={cn('flex items-center py-2 text-sm font-medium transition-all duration-150', collapsed ? 'px-0 justify-center' : 'px-4 gap-3', isActive ? 'text-jarvis-cyan bg-jarvis-cyan/10 border-r-2 border-jarvis-cyan' : 'text-jarvis-muted hover:text-jarvis-text hover:bg-white/5')}>
